...
function inviteStatus(invite: Invite) {
  if (invite.usedAt)
    return { label: "Utilizado", className: "bg-blue-400/10 text-blue-200" };
  if (invite.revokedAt)
    return { label: "Revogado", className: "bg-red-400/10 text-red-200" };
  if (new Date(invite.expiresAt) <= new Date())
    return { label: "Expirado", className: "bg-amber-400/10 text-amber-200" };
...
export default function InviteManager({
  ...
  async function revokeInvite(id: string) {
    if (
      !window.confirm(
        "Revogar este convite? Ele não poderá mais ser utilizado.",
      )
    )
      return;
    setError("");
    ...
    <main className="min-h-screen bg-[#222220] px-4 py-6 text-white sm:px-6">
      <div className="mx-auto grid w-full max-w-6xl gap-5">
        <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-white/55">
              Gere códigos individuais e acompanhe sua utilização.
            </p>
          </div>
        </header>
        ...
        {error && (
          <div
            role="alert"
            className="rounded-md border border-red-400/30 bg-red-950/30 px-4 py-3 text-sm text-red-100">
            {error}
          </div>
        )}
        ...
        <section className="rounded-md border border-white/10 bg-[#171717] p-5">
          <div className="mb-5">
            ...
            <p className="mt-1 text-xs text-white/45">
              O código completo aparece uma única vez. Envie-o apenas para a
              pessoa convidada.
            </p>
          </div>
          <form
            ...
            <div className="grid gap-2">
              ...
              <select
                ...
                value={role}
                onChange={(event) =>
                  setRole(event.target.value as "BARBER" | "ADMIN")
                }
            ...
            <button
              disabled={submitting}
              className="h-11 rounded-md bg-[#b9ff62] px-5 text-sm font-black text-black disabled:opacity-60">
          ...
          {generatedCode && (
            <div className="mt-5 rounded-lg border border-[#b9ff62]/30 bg-[#b9ff62]/5 p-4">
              <p className="text-[10px] font-black uppercase tracking-wider text-[#b9ff62]">
                Código gerado — copie agora
              </p>
              <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-center">
                <code className="flex-1 rounded-md bg-black/30 px-4 py-3 text-center text-lg font-black tracking-[0.12em] text-white">
                  {generatedCode}
                </code>
                <button
                  type="button"
                  onClick={copyCode}
                  className="h-11 rounded-md border border-[#b9ff62]/30 px-5 text-xs font-black text-[#b9ff62]">
              ...
              <table className="w-full min-w-190 text-left text-xs">
                <thead className="bg-[#101010] text-[10px] uppercase tracking-wider text-white/40">
                  <tr>
                    ...
                    <th className="px-5 py-3">Utilizado por</th>
                ...
                <tbody className="divide-y divide-white/5">
                  {invites.map((invite) => {
                    ...
                    return (
                      <tr key={invite.id} className="text-white/65">
                        ...
                        <td className="px-5 py-4">
                          <span
                            className={`rounded px-2 py-1 text-[10px] font-black uppercase ${status.className}`}>
                            {status.label}
                          </span>
                        </td>
                        <td className="px-5 py-4">
                        ...
                        <td className="px-5 py-4 text-right">
                          {canRevoke && (
                            <button
                              ...
                              className="font-black text-red-200 hover:underline">
                              Revogar
                            </button>
                          )}
                        </td>
